/**
 * Created by Pngyul on 2018/04/01
 */
window.onload = function() {
    // 获取元素
    function $(id) {return document.getElementById(id);}
    var js_slider = $("sliders");  // 获取最大盒子
    var slider_main_block = $("slider_main_block");  // 滚动图片的父亲
    var imgs = slider_main_block.children;  // 获得所有的图片组 需要滚动的部分
    var circles = $("circles");  // 获得 控制span 的 父盒子
    var ul=circles.children[0];
    var arrows = $("arrows");
    // 操作元素
    // 生成小span
    ...
    for(var i=0;i<imgs.length; i++) {
        ...
        var li = document.createElement("li");// 创建 span
        li.innerHTML = i+1;  //  6 - 0     6 - 1   // 实现 倒序 的方式插入
        ul.appendChild(li);  // 再 父亲 倒数第二个盒子的前面插入
    }

    // 下面的第一个小span  是默认的蓝色
    var lis = ul.children;   // 得到所有的 span

    var spans= arrows.children;

    lis[0].setAttribute("class","current");  // 两个类名
    ...
    for(var k in spans){
        spans[k].onclick = function() {
            if (this.className == "arrow-left") { // 判断当前点击的这个按钮是不是 prev
                // alert("您点击了左侧按钮");
            ...
            else if (this.className == "arrow-right") {  // 右侧按钮开始
                autoplay();
            }
        }
    }
    for(var k in lis){   //   k  是索引号  spans[k]    spans[0]  第一个span
    ...
    function setSquare() {
        ...
        for(var i=0;i<lis.length;i++){   //  8个span   我们要 1-6  不要 7  索引号
            lis[i].className = "";
        }
        lis[iNow].className = "current";  // 记住 + 1
    }
    // 定时器开始  其实， 定时器就是  右侧按钮
    var timer = null;
